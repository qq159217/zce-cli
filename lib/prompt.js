...
module.exports = (files, metalsmith, next) => {
  ...
  // const getRemoteUrl = () => exec('git config --local --get remote.origin.url').toString().trim()

  const setDefaults = item => {
    switch (item.name) {
      case 'name':
        item.default = item.default || path.basename(metadata.dest)
      ...
      case 'repository':
        ...
        // item.default = item.default || getRemoteUrl()
        break
    }
    return item
  }
